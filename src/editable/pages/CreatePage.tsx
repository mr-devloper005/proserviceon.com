      ...
      <main className="min-h-screen bg-[var(--editable-page-bg,#fff7ee)] text-[var(--editable-page-text,#2f1d16)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-lg border border-[var(--editable-border)] bg-white p-6 shadow-sm lg:grid-cols-[0.85fr_1.15fr] lg:p-8">
            <aside>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {enabledTasks.map((item) => {
                  ...
                  return (
                    <button key={item.key} type="button" onClick={() => setTask(item.key)} className={`rounded-lg border p-4 text-left transition ${active ? 'border-[#2d275b] bg-[#2d275b] text-white' : 'border-[var(--editable-border)] bg-white text-slate-800 hover:-translate-y-0.5'}`}>
            ...
            <form onSubmit={submit} className="rounded-lg border border-[var(--editable-border)] bg-[#f7f8fb] p-5 sm:p-7">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="rounded-md bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-slate-700">{session.name}</span>
              </div>
              ...
              <div className="mt-6 grid gap-4">
                ...
                <input className={fieldClass} value={image} onChange={(event) => setImage(event.target.value)} placeholder="Business photo or logo URL" />
                ...
                <textarea className={`${fieldClass} min-h-48`} value={body} onChange={(event) => setBody(event.target.value)} placeholder="Services, location, contact notes, hours, and business description" required />
              </div>
